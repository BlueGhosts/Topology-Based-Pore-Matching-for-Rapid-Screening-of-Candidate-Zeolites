#!perl
# Beta V1.2 by wangjiaze in 2025-03-03  debugV1.1 need $leftangle >= $thresholds to delete
	...
	foreach my $atom (@$atoms) {
	    ...
	    if ( $element eq $poreElement and $numBonds == 2){
			...
			if ( $angle >= $thresholds ){	
				...
				my $leftmonitor = $doc->CreateAngle([@$connectedAtoms[0], $leftAtom, @$connectedAtoms[1]]);
	    		my $leftangle = $leftmonitor->Angle;
	    		$leftmonitor->Delete;
				while ($leftAtom->NumBonds==2 and $leftangle >= $thresholds){
					$leftConnectedAtoms = $leftAtom->AttachedAtoms;
					
					$leftmonitor = $doc->CreateAngle([@$connectedAtoms[0], $leftAtom, @$connectedAtoms[1]]);
	    			$leftangle = $leftmonitor->Angle;
	    			$leftmonitor->Delete;
