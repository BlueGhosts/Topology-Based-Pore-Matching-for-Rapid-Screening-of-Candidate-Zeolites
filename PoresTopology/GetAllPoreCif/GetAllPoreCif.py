...
def GetAllPoreCif(adoPath, cifPath, outCifPath):
    adoFiles = []
    cifFiles = []
        ...
        if len(TD10s) > 0:
            for order in range(len(TD10s)):
                ...
                if len(TD10s) == 1:
                    ...
                    outCifFilepath = f'{outCifPath}/{name}_{order+1}.cif'
                WriteCif(crystal, outCifFilepath)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        GetAllPoreCif(adoPath, cifPath, outCifPath)
    else:
